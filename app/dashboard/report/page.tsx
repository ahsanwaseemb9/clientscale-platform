...
export default function AuditReportPage() {
  ...
  const VercelGraph = ({ title, series, yUnit = '', xLabels = ['6h ago', '2m ago'], maxVal = 100 }: any) => {
    ...
    // Explicit Formatter to force line breaks on specific labels ONLY for mobile displays
    const formatLabel = (label: string) => {
      if (label === 'TBT (Thread Lock)') {
        return (
          <>
            <span className="sm:hidden">TBT<br/>(Thread Lock)</span>
            <span className="hidden sm:inline">TBT (Thread Lock)</span>
          </>
        );
      }
      ...
      if (parts.length > 1) {
        return (
          <>
            <span className="sm:hidden">{parts[0]}<br/>{parts.slice(1).join(' ')}</span>
    ...
    return (
      <div className="bg-white border border-gray-200 rounded-xl p-4 sm:p-6 w-full flex flex-col h-full min-h-[260px] shadow-sm relative overflow-hidden">
        <div className="flex justify-between items-start mb-4 sm:mb-6">
          <h3 className="text-base sm:text-lg font-bold text-gray-900 tracking-tight leading-tight flex items-start gap-1.5 sm:gap-2">
            {title}
        ...
        <div className="flex flex-row gap-4 sm:gap-8 mb-6 sm:mb-8 relative z-10 w-full">
          {series.map((s: any, idx: number) => (
            <div key={idx} className="flex flex-col gap-1 sm:gap-1.5 flex-1">
              <span className="text-xs sm:text-sm text-gray-500 font-semibold leading-snug break-words">
                {formatLabel(s.label)}
              </span>
              <div className="flex items-center gap-2 text-sm sm:text-xl font-mono font-bold text-gray-900">
        ...
        <div className="relative flex-grow flex items-end">
          <div className="absolute inset-0 flex flex-col justify-between pb-6">
            {[maxVal, maxVal * 0.75, maxVal * 0.5, maxVal * 0.25, 0].map((val, i) => (
              <div key={i} className="flex items-center w-full gap-2 sm:gap-3 relative z-0">
                <span className="text-[10px] sm:text-xs font-mono text-gray-400 w-8 sm:w-10 text-right shrink-0">{val}{yUnit}</span>
          ...
          <div className="absolute bottom-0 left-11 sm:left-14 right-0 flex justify-between pt-2 border-t border-gray-200">
            {xLabels.map((l: string, i: number) => (
              <span className="text-[10px] sm:text-xs font-mono text-gray-400" key={i}>{l}</span>
            ))}
          </div>
        </div>
      </div>
